...
- **R 键**：一键回到 100% 原始比例
- **双击图片**：非 100% 时回到 100%
- **左下角百分比指示器**：缩放时显示当前比例（如 150%），回到 100% 后自动隐藏
- **多图切换**：不影响 X 原生箭头切换，100% 时完全穿透点击
- **不影响右侧评论区**：覆盖层仅位于左侧图片区域
...
- 通过 CSS transform 对 X 原图创建黑色覆盖层实现缩放和拖拽
- 覆盖层仅覆盖左侧图片区域，右侧评论区及相关按钮不受影响
- 利用 `overflow: visible` 保证放大后的图片不会溢出被裁切
- 定时器轮询 + 路由变化检测 `/photo/N` 地址变化
